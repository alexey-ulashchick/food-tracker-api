...
export function CalorieChart({ days }: { days: ChartDay[] }) {
  ...
  // dead space either side, and pick() went on mixing a real client rect with
  // the stale step — the first scrub after a resize selected the wrong day.
  ...
  return (
    <div ref={frameRef} style={{ position: 'relative', width: '100%', height: CHART_HEIGHT }}>
      {/* Nothing is drawn until the frame has been measured: at width 0 every
          bar, tick and scrub position would be wrong, and the geometry is
          cheap enough to simply wait one commit for. */}
      {width > 0 ? (
        <svg
          ref={svgRef}
          width="100%"
