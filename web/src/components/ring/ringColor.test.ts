import { overage, palette } from '@/theme/tokens'
...
const protein = palette.protein.map(hexToRgb)
const RED = hexToRgb(overage)
...
describe('ringColor', () => {
  test('lap 1 is the plain palette gradient', () => {
    expect(ringColor(protein, 0)).toEqual(protein[0])
    expect(ringColor(protein, 1)).toEqual(protein[1])
  })

  test('past the ramp it is solid systemRed', () => {
    expect(ringColor(protein, OVERAGE_END_T)).toEqual(RED)
    expect(ringColor(protein, 1.5)).toEqual(RED)
    expect(ringColor(protein, 4)).toEqual(RED)
  })

  // Both knees must be continuous or the ring shows a hard colour step.
  test('is continuous at t = 1', () => {
    const eps = 1e-6
    expect(sameColor(ringColor(protein, 1 - eps), ringColor(protein, 1 + eps))).toBe(true)
  })

  test('is continuous at t = OVERAGE_END_T', () => {
    const eps = 1e-6
    expect(
      sameColor(ringColor(protein, OVERAGE_END_T - eps), ringColor(protein, OVERAGE_END_T + eps)),
    ).toBe(true)
  })

  test('ramps monotonically towards red across lap 2', () => {
    const start = ringColor(protein, 1)
    const mid = ringColor(protein, 1 + (OVERAGE_END_T - 1) / 2)
  ...
  test('a shared segment boundary yields identical colours', () => {
    for (const t of [0.25, 0.5, 0.75, 1, 1.02, 1.07, 2]) {
      expect(ringColor(protein, t)).toEqual(ringColor(protein, t))
    }
  })
})
...
describe('gradientStops', () => {
  test('a segment with no interior knee gets exactly two stops', () => {
    const s = gradientStops(protein, 0, 0.25)
  ...
  test('inserts a stop where t = 1 falls inside the segment', () => {
    const s = gradientStops(protein, 0.75, 1.25)
  ...
  test('inserts a stop for the ramp end too', () => {
    const s = gradientStops(protein, 1, 1.25)
  ...
  test('does not insert a knee that sits on the segment boundary', () => {
    ...
    expect(gradientStops(protein, 1, 1.5).filter((s) => s.location === 0)).toHaveLength(1)
    expect(gradientStops(protein, 0.5, 1)).toHaveLength(2)
  })
  ...
  test('locations are ascending and span the full [0, 1]', () => {
    for (const [a, b] of [
      ...
      const s = gradientStops(protein, a!, b!)
      expect(s[0]!.location).toBe(0)
  ...
  test('endpoint colours agree with ringColor at the same t', () => {
    const s = gradientStops(protein, 0.75, 1.25)
    expect(s[0]!.color).toEqual(ringColor(protein, 0.75))
    expect(s.at(-1)!.color).toEqual(ringColor(protein, 1.25))
  })
})
